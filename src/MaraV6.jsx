...
const IMAGE_CATALOG = [
  ...
  {"id":"billow-strand-blue-01","pattern":"Billow","sector":"Hospitality","application":"Restaurant Feature","lighting":"Blue RGB Backlit","color":"Glacier White","mood":["dramatic","cool","aquatic"],"keywords":["billow","restaurant","blue","RGB","strand","staircase"],"rgb_options":true,"description":"The Strand House with blue RGB backlighting — same installation, different mood. Shows RGB versatility.","image_url":""},
...
const getPlaceholderStyle = (id) => {
  const colors = {
    billow: { bg: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', accent: '#e94560' },
    seattle: { bg: 'linear-gradient(135deg, #2d3436 0%, #636e72 100%)', accent: '#74b9ff' },
    greatwave: { bg: 'linear-gradient(135deg, #0c2461 0%, #1e3799 50%, #4a69bd 100%)', accent: '#82ccdd' },
    brick: { bg: 'linear-gradient(135deg, #2c2c2c 0%, #1a1a1a 100%)', accent: '#cd9777' },
    marilyn: { bg: 'linear-gradient(135deg, #2d132c 0%, #801336 100%)', accent: '#ee4540' },
    buddha: { bg: 'linear-gradient(135deg, #1a1a2e 0%, #2d2d44 100%)', accent: '#ffd700' },
    fins: { bg: 'linear-gradient(135deg, #0f0f0f 0%, #1a1a1a 50%, #2a2a2a 100%)', accent: '#00d9ff' },
    flame: { bg: 'linear-gradient(135deg, #1a0a00 0%, #2d1810 50%, #4a2c2a 100%)', accent: '#ff6b35' },
    desert: { bg: 'linear-gradient(135deg, #1a1510 0%, #2d2418 100%)', accent: '#f39c12' },
    sanddune: { bg: 'linear-gradient(135deg, #1a1a1a 0%, #2d2d3a 100%)', accent: '#a29bfe' }
  };
  const key = Object.keys(colors).find(k => id.toLowerCase().includes(k)) || 'fins';
  return colors[key];
};
...
export default function MaraV6() {
  ...
  const style = currentImage ? getPlaceholderStyle(currentImage.id) : null;
      ...
      <div className="w-1/2 h-full relative flex items-center justify-center p-6">
        {currentImage ? (
          <div 
            ...
            style={{ 
              background: currentImage.image_url 
                ? `url(${currentImage.image_url}) center/cover` 
                : style.bg 
            }}
          >
            {/* Accent glow for placeholder */}
            {!currentImage.image_url && (
              <div 
                className="absolute inset-0 opacity-20"
                style={{ background: `radial-gradient(circle at 30% 70%, ${style.accent}40 0%, transparent 50%)` }}
              />
            )}
            
            {/* Pattern letter for placeholder */}
            {!currentImage.image_url && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-8xl font-thin" style={{ color: style.accent }}>
                  {currentImage.pattern.charAt(0)}
                </div>
              </div>
            )}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
              <div className="flex items-center gap-2 mb-3">
                ...
                {currentImage.rgb_options && (
                  <span className="text-xs px-2.5 py-1 rounded-full border" style={{ background: `${style?.accent || '#a29bfe'}20`, borderColor: `${style?.accent || '#a29bfe'}40`, color: style?.accent || '#a29bfe' }}>RGB</span>
                )}
              </div>
            ...
            <div className="absolute top-6 left-6 right-6 flex justify-between items-start">
              <div>
                <h2 className="text-3xl font-light tracking-tight text-white/90">{currentImage.pattern}</h2>
                <p className="text-sm text-white/40 mt-1">{currentImage.application}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-white/30">{currentImage.color}</p>
                <p className="text-xs text-white/30">{currentImage.lighting}</p>
              </div>
            </div>
          </div>
        ) : (
