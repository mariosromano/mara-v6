...
// ═══════════════════════════════════════════════════════════════════════════════
// IMAGE CATALOG - Accurate labels, mood tags, strict backlight boolean
// ═══════════════════════════════════════════════════════════════════════════════
...
const IMAGE_CATALOG = [
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    isBacklit: false,
    keywords: ['industrial', 'brick', 'carbon', 'concrete', 'dark', 'grey', 'gray', 'aviation', 'airport', 'terminal'],
  ...
  },
  {
    ...
    isBacklit: false,
    keywords: ['industrial', 'brick', 'dove', 'grey', 'gray', 'warm', 'soft', 'aviation', 'airport'],
  ...
  },
  {
    ...
    isBacklit: false,
    keywords: ['industrial', 'brick', 'neutral', 'concrete', 'light', 'grey', 'gray', 'aviation', 'airport'],
  ...
  },
  {
    ...
    isBacklit: false,
    keywords: ['industrial', 'brick', 'artista', 'mist', 'grey', 'gray', 'subtle', 'movement', 'aviation'],
  ...
  },
  {
    ...
    description: 'Industrial Brick in Laguna — bold deep blue. Makes a real statement. Great for branding.'
  },
  {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    isBacklit: false,
    keywords: ['seattle', 'tile', 'modular', 'healthcare', 'hospital', 'corridor', 'wave', 'flat'],
    ...
    description: 'Seattle modular tiles — alternating carved wave and flat panels. Perfect for healthcare corridors.'
  },
  ...
  },
  {
    ...
    description: 'Great Wave as restaurant feature wall — photographs beautifully for social media.'
  },
  {
    id: 'greatwave-lobby',
    pattern: 'Great Wave',
    ...
    description: 'Great Wave in corporate lobby — makes a statement about creativity and boldness.'
  },
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    isBacklit: false,
    keywords: ['brick', 'water', 'fountain', 'pool', 'waterfall', 'outdoor', 'tropical', 'black'],
    ...
    description: 'Brick pattern water feature in black — horizontal carved lines channel water into waterfalls.'
  },
  {
    id: 'brick-water-2',
    pattern: 'Brick',
  ...
  },
  {
    ...
    isBacklit: true,
    keywords: ['brick', 'water', 'backlit', 'backlight', 'glow', 'night', 'dramatic', 'rgb', 'white'],
    ...
    description: 'Brick with backlighting AND water — light glows through carved channels as water cascades.'
  },
  {
    id: 'brick-water-4',
    pattern: 'Brick',
    ...
    isBacklit: false,
    keywords: ['brick', 'water', 'night', 'evening', 'lighting', 'black', 'ambient'],
    ...
    description: 'Brick water feature at night — uplighting catches the spray against black surface.'
  },
  {
    id: 'brick-water-5',
    pattern: 'Brick',
    ...
    isBacklit: false,
    keywords: ['brick', 'water', 'day', 'daylight', 'natural', 'pool', 'black', 'sunlight'],
    ...
    description: 'Brick water feature in bright daylight — texture catches sun, water sparkles.'
  },
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    isBacklit: true,
    keywords: ['buddha', 'zen', 'meditation', 'spiritual', 'calm', 'spa', 'wellness', 'peaceful', 'asian', 'mandala', 'backlit', 'backlight', 'glow'],
    ...
    description: 'Buddha mandala — custom carved with intricate detail. Backlit for ethereal glow. Perfect for spa, meditation room.'
  },
  {
    id: 'buddha-2',
    pattern: 'Buddha Mandala',
    ...
    isBacklit: true,
    keywords: ['buddha', 'restaurant', 'asian', 'zen', 'dining', 'feature wall', 'backlit', 'backlight'],
  ...
  },
  {
    ...
    isBacklit: true,
    keywords: ['flame', 'pink', 'rgb', 'backlit', 'backlight', 'bedroom', 'romantic', 'dramatic', 'glow'],
    ...
    description: 'Flame with pink RGB backlighting — dramatic, romantic atmosphere. Color shifts throughout the evening.'
  },
  {
    id: 'flame-lobby',
    pattern: 'Flame',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    isBacklit: true,
    keywords: ['desert', 'sunset', 'cactus', 'arizona', 'southwest', 'scottsdale', 'hotel', 'resort', 'regional', 'backlit', 'backlight'],
    ...
    description: 'Desert Sunset — saguaro cactus silhouettes against carved mountain ridges. Regional identity for Southwest hospitality.'
  },
  {
    id: 'desert-sunset-2',
    pattern: 'Desert Sunset',
  ...
  },
  {
    ...
    description: 'Desert Sunset abstract — simplified mountain forms. Southwest aesthetic without being literal.'
  },
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    description: 'Sand Dune pattern thermoformed into dramatic curved column. Black Corian, sculptural entry statement.'
  }
];

// ═══════════════════════════════════════════════════════════════════════════════
// SMART FAMILY GROUPING - 3 Tiers of Intelligence
// ═══════════════════════════════════════════════════════════════════════════════

const getFamilyImages = (selectedImage) => {
  if (!selectedImage) return [];
  
  // TIER 1: Same pattern family, different colors (Industrial Brick)
  if (selectedImage.patternFamily) {
    ...
    if (colorVariants.length >= 3) {
      return colorVariants.slice(0, 4);
    }
  }
  
  // TIER 2: Same pattern, different applications (Great Wave variations)
  const patternVariants = IMAGE_CATALOG.filter(
    img => img.pattern === selectedImage.pattern && img.id !== selectedImage.id
  );
  if (patternVariants.length >= 3) {
    return patternVariants.slice(0, 4);
  }
  
  // TIER 3: Similar attributes (mood, enhancement, sector)
  const similar = IMAGE_CATALOG.map(img => {
    if (img.id === selectedImage.id) return { ...img, score: -1 };
    
    let score = 0;
    
    // Same enhancement (backlit → backlit)
  ...
  // Fallback if not enough similar
  if (similar.length < 2) {
...
// ═══════════════════════════════════════════════════════════════════════════════
// SEARCH FUNCTION - Strict backlight filtering
// ═══════════════════════════════════════════════════════════════════════════════
...
const searchImages = (query) => {
  ...
  return scored
    ...
    .sort((a, b) => b.score - a.score)
    .slice(0, 2); // Only 2 results in chat
};
...
## RULES
1. MAX 50 words
2. Use [Image: id] tags for images
3. ONE question per response
4. Show max 2 images per response

## BACKLIGHT IMAGES ONLY (isBacklit: true)
- buddha-1, buddha-2 (Buddha Mandala)
- brick-water-3 (Brick Backlit + Water)
- flame-pink (Flame Pink RGB)
- desert-sunset-1 (Desert Sunset Cactus)

## WHEN USER ASKS "BACKLIGHT"
ONLY show images from the backlit list above. Never show non-backlit images for backlight queries.

## OTHER IMAGE IDS
- Industrial Brick: industrial-brick-carbon, industrial-brick-dove, industrial-brick-neutral, industrial-brick-artista, industrial-brick-laguna, industrial-brick-verdant
...
- Custom: marilyn-1, marilyn-2

Remember: You're Mara. Warm, brief. Tap an image to see the family.`;
...
export default function MaraV13() {
  ...
  // Handle image click → show family
  ...
  const send = async (text, contextImage = null) => {
    ...
    setMessages(m => [...m, {
      role: 'assistant',
      text: responseText,
      images: responseImages.slice(0, 2) // Max 2 images
    }]);
  ...
  return (
    <div className="h-screen bg-stone-950 text-stone-100 flex flex-col" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
          ...
          >
            ...
            <div className="p-4 overflow-y-auto flex-1">
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="bg-stone-900 border border-stone-800 rounded-xl p-3 flex-1">
                  <p className="text-sm text-stone-300">{selectedImage.description}</p>
                  <p className="text-sm text-stone-500 mt-2">Tap any image below to see full specs.</p>
                </div>
              </div>

              {/* Family Grid - 4 smaller images */}
              ...
              {/* View specs for selected */}
              <button
                onClick={() => handleFamilyClick(selectedImage)}
                className="mt-4 w-full py-3 bg-stone-800 hover:bg-stone-700 rounded-xl text-sm font-medium transition-colors"
              >
      ...
      {specsImage && (
        ...
        >
          <div 
            className="bg-stone-950 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-auto border border-stone-800"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Image */}
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  <p className="text-xs text-stone-500 uppercase tracking-wide">Color</p>
                ...
                <div>
                  ...
                  <p className="text-sm text-stone-200">{specsImage.specs.maxPanel || `${specsImage.specs.height} × ${specsImage.specs.width}`}</p>
                </div>
                <div>
                  <p className="text-xs text-stone-500 uppercase tracking-wide">Lead Time</p>
                  <p className="text-sm text-stone-200">{specsImage.specs.leadTime}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-stone-200">${specsImage.specs.pricePerSF}/SF</p>
                </div>
                {specsImage.specs.enhancement && (
                  <div className="col-span-2">
                    <p className="text-xs text-stone-500 uppercase tracking-wide">Enhancement</p>
              ...
              {/* CTAs */}
              <div className="flex gap-3">
                <button className="flex-1 py-3 bg-stone-800 hover:bg-stone-700 rounded-xl font-medium text-sm transition-colors border border-stone-700">
                  Download Specs
                </button>
                <button className="flex-1 py-3 bg-stone-100 text-stone-900 hover:bg-white rounded-xl font-medium text-sm transition-colors">
                  Request Quote
                </button>
              </div>
              ...
              {specsImage.shopDrawing && (
                <a 
                  ...
                  className="block mt-4 text-center text-sm text-stone-400 hover:text-stone-200 underline"
                >
                  View Shop Drawing →
                </a>
              )}
              ...
              <button
                onClick={closeSpecs}
                className="mt-4 w-full py-2 text-sm text-stone-500 hover:text-stone-300"
              >
