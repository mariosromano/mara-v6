...
const IMAGE_CATALOG = [
  ...
  { id: 'buddha-1', pattern: 'Custom', color: 'white', keywords: ['buddha', 'zen', 'meditation', 'spiritual', 'calm', 'spa', 'wellness', 'peaceful'], image: `${CLOUDINARY_BASE}/spa-_Buddha_2_zid08z.png` },
...
CRITICAL: Keep responses under 50 words. 2-3 sentences max. Ask ONE follow-up question.
...
- Install: InterlockPanel™ puzzle-piece system, 1/32" tolerance
  ...
  const findBestImage = () => {
    ...
    for (const item of IMAGE_CATALOG) {
      ...
      for (const term of searchTerms) {
        ...
        // Check enhancement match
        if (item.enhancement && item.enhancement.toLowerCase().includes(term)) {
          score += 12;
        }
      }
      
      // Boost for exact pattern match from selector
